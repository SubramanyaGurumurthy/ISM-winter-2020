...
def assymetry(image):
    ...
    ret, thresh = cv.threshold(
        blur, 70, 255, cv.THRESH_BINARY_INV+cv.THRESH_OTSU)
    ...
    ellipse_pnts = cv.ellipse2Poly((int(ellipse[0][0]), int(ellipse[0][1])), (int(
        ellipse[1][0]), int(ellipse[1][1])), int(ellipse[2]), 0, 360, 1)
...
def Moments(imageID, imageSelected):
    moments = []
    ret, thresh = cv.threshold(
        imageSelected, 128, 255, cv.THRESH_BINARY_INV | cv.THRESH_OTSU)
    m = cv.moments(thresh)
...
def image_feature_extraction(begin, end):
    ...
    reader = pd.read_csv(
        "D:\Documents\GitHub\TUHH_ISM_tumor_detection\csv_features\\temp.csv", skiprows=begin, nrows=end)
    ...
    for row in range(len(reader)):
        ...
        if path.exists(PATH):
            ...
            # Change Constrast
            constrst_change_img = processor.change_contrast(
                circular_mask_img, 5)

            # Write Processed Image
            cv.imwrite('G:\ISM\ISM\\Data\\Processed_Test_Image_18122020\\' +
                       imageID+'.jpg', constrst_change_img)
            ...
            segmented_image = processor.k_means_segementation(
                constrst_change_img, 5)
            ...
            with open("D:\Documents\GitHub\TUHH_ISM_tumor_detection\csv_features\\features_training_18122020.csv", "a", newline='') as csvfile:
                ...
                writer.writerow([imageID, colorhist_data[0], colorhist_data[1], colorhist_data[2], colorhist_data[3], colorhist_data[4], colorhist_data[5], colorhist_data[6], colorhist_data[7], colorhist_data[8], colorhist_data[9], colorhist_data[10], colorhist_data[11],
                                 texture_data[0], texture_data[1], texture_data[2], texture_data[
                                     3], texture_data[4], texture_data[5], texture_data[6], texture_data[7],
                                 texture_data[8], texture_data[9], texture_data[10], texture_data[
                                     11], texture_data[12], texture_data[13], texture_data[14], texture_data[15],
                                 hu_moments_data[0], hu_moments_data[1], hu_moments_data[2], hu_moments_data[
                    3], hu_moments_data[4], hu_moments_data[5], hu_moments_data[6],
...
if __name__ == '__main__':
    ...
    three = multiprocessing.Process(
        target=image_feature_extraction, args=[6, 3])
    three.start()

    four = multiprocessing.Process(
        target=image_feature_extraction, args=[9, 3])
    four.start()

    five = multiprocessing.Process(
        target=image_feature_extraction, args=[12, 3])
    five.start()

    six = multiprocessing.Process(
        target=image_feature_extraction, args=[15, 3])
    six.start()

    seven = multiprocessing.Process(
        target=image_feature_extraction, args=[18, 4])
    seven.start()

    eigth = multiprocessing.Process(
        target=image_feature_extraction, args=[22, 4])
    eigth.start()

    nine = multiprocessing.Process(
        target=image_feature_extraction, args=[26, 4])
    nine.start()

    ten = multiprocessing.Process(
        target=image_feature_extraction, args=[30, 5])
    ten.start()

    eleven = multiprocessing.Process(
        target=image_feature_extraction, args=[35, 7])
    eleven.start()
